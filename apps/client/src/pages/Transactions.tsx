...
export default function Transactions({ type }: TransactionsProps) {
  ...
  const displayFrom = period === 'custom' ? dateFrom : format(range.start, 'yyyy-MM-dd');
  const displayTo = period === 'custom' ? dateTo : format(range.end, 'yyyy-MM-dd');
  ...
  const handleDateChange = (which: 'from' | 'to', value: string) => {
    ...
    setDateFrom(which === 'from' ? value : format(preset.start, 'yyyy-MM-dd'));
    setDateTo(which === 'to' ? value : format(preset.end, 'yyyy-MM-dd'));
    setPeriod('custom');
  };
  ...
  return (
    <div className="space-y-5 pb-4">
      ...
      <div className="space-y-2">
        <Tabs value={period} onValueChange={(v) => selectPreset(v as Period)}>
        ...
        <Button
          variant="ghost"
          size="sm"
          className={`w-full justify-start font-bold ${isCustom ? 'text-accent' : 'text-muted-foreground'}`}
