...
**Диапазон дат (страницы Расходы/Доходы):** один **эффективный диапазон** управляет всей страницей
(итог, график, разбивка по категориям, история). Вкладки День/Неделя/Месяц/Год — **пресеты** диапазона
относительно «сейчас». Блок **«Свой период»** (поля «Дата с / по») переводит страницу в режим `custom`:
вкладки снимаются, всё пересчитывается под выбранные даты. Открытая граница = пустая дата
(«с» → от первой операции, «по» → до сегодня); обе пустые → «за всё время» (все операции). Гранулярность
графика выбирается по длине диапазона (часы → дни → недели → месяцы), а не по enum-пресету. Доп. фильтр
истории по сумме (от/до) — отдельно, в шапке «История».
...
- **Редактирование транзакций**: `PATCH /transactions/:id` (откат+применение баланса атомарно, пересчёт снимка курса) + клиентский `EditTransactionDialog` (правка/удаление); единый диапазон дат (пресеты + «Свой период») управляет всей страницей + фильтр истории по сумме *(Этап 6)*
...
- ✅ Единый диапазон дат на странице `Transactions`: вкладки-пресеты (День/Неделя/Месяц/Год) + «Свой период» (произвольные «с/по», открытые границы → «за всё время»). Диапазон управляет всей страницей (итог/график/категории/история), гранулярность графика — по длине диапазона. Доп. фильтр истории по сумме (от/до).
