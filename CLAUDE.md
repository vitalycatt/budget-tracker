...
- **Telegram-бот** — пользователь пишет в чат **текстом** («потратил 500 на кофе»,
  ...
  - **Карточка-черновик (единая точка подтверждения).** После парсинга бот не сохраняет сразу,
    а показывает черновик с полями (тип/сумма/счёт/категория/дата/описание) и инлайн-кнопками
    **Сохранить/Отмена** + правка любого поля. Это страховка от ошибок LLM и **общий экран
    «проверь и поправь» для всех способов ввода** (текст сейчас; голос и пошаговый мастер — позже
    приземляются в ту же карточку). После сохранения — кнопка **↩️ Отменить** (атомарный откат баланса).
...
- **8.1** ✅/🚧:
  ...
  - ✅ Карточка-черновик: после парсинга бот шлёт черновик (тип/сумма/счёт/категория/дата/описание) с инлайн-кнопками **Сохранить / Отмена** и правкой каждого поля (сумма/описание — текстом; тип/счёт/категория/дата — подменю кнопок; «новая категория» — текстом). Счёт фильтруется по `!isArchived`; при 1 счёте — предвыбран, при нескольких — выбор в карточке. Сохранение блокируется без счёта.
