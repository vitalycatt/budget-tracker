...
1. **Telegram-бот (текст)** — приём текстовых сообщений, **парсинг через LLM (Claude API)** → создание транзакции, подтверждение пользователю. Голос — позже. *(Этап 4)*
2. **Приведение к 3-страничной структуре** (раздел 3) — убрать страницу категорий, создание на лету. *(Этап 5)*
3. Редактирование транзакций (сейчас только удаление); фильтры по дате/сумме. *(Этап 6)*
4. *(позже)* Голосовой ввод в боте (audio → текст).
